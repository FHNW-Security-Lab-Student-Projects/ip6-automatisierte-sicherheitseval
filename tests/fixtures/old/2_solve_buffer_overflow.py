...
proj = angr.Project(
    "tests/fixtures/2_buffer_overflow", auto_load_libs=False
)  # angr takes binary as input and creates a project object
init_state = proj.factory.full_init_state(stdin=angr.SimFileStream("stdin", size=80))
...
simulation = proj.factory.simulation_manager(
    init_state
...
if simulation.found:
    print(
        simulation.found
    )  # should print the found states that reached the successful condition, e.g., "SimState(0x8048080)"
    print(
        simulation.found[0]
    )  # should print the first found state, e.g., "SimState(0x8048080)"
    ...
    result = subprocess.run(
        ["./tests/fixtures/2_buffer_overflow"], input=solution, capture_output=True
    )
