...
proj = angr.Project(
    "tests/3_buffer_overflow_strcpy", auto_load_libs=False, use_sim_procedures=True
)

# symbolisches Argument
arg = claripy.BVS("arg", 200 * 8)

init_state = proj.factory.full_init_state(args=["tests/3_buffer_overflow_strcpy", arg])
...
if simulation.found:
    ...
    result = subprocess.run(
        ["./tests/3_buffer_overflow_strcpy"], input=solution, capture_output=True
    )
